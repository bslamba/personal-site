...
export async function POST(request: Request) {
  ...
  const s = await getSession(jar.get(VAULT_COOKIE)?.value)
  if (!s) return NextResponse.json({ error: 'Unauthorised' }, { status: 401 })
  const b = await request.json().catch(() => ({})) as { firstName?: string; lastName?: string; email?: string; avatar?: string }
  ...
  const u = await findUser(s.u)
  return NextResponse.json({ ok: true, user: u ? publicUser(u) : null })
}
